...
class _BatchConfigDialogState extends State<BatchConfigDialog> {
  ...
  void initState() {
    super.initState();
    // Default to the last translated chapter so the batch can resume from
    // the most recent completed chapter. Fall back to the first pending
    // chapter when no chapter has been translated yet.
    _startIndex = widget.project.chapters.lastIndexWhere(
      (c) => c.status == ChapterStatus.done,
    );
    if (_startIndex == -1) {
      _startIndex = widget.project.chapters.indexWhere(
        (c) => c.status != ChapterStatus.done,
      );
    }
